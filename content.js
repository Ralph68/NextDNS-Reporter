 ...
 *
 * Ce script s'injecte sur TOUTES les pages mais s'arrête immédiatement à la
 * première ligne si la page n'est pas une page de blocage NextDNS.
...
(() => {
  ...
  // ---------------------------------------------------------------------------
  // 1. DÉTECTION — Sortie immédiate si ce n'est pas une page de blocage NextDNS
  //    Les trois sélecteurs sont exclusifs aux pages de blocage NextDNS.
  //    Aucune page normale ne les contient simultanément.
  // ---------------------------------------------------------------------------
  if (
  ...
  // 2. CONSTANTES
  // ---------------------------------------------------------------------------
  const BLOCK_EXPIRY_MS = 48 * 60 * 60 * 1000; // Délai avant re-signalement (48h)
  const SEND_TIMEOUT_MS = 10_000;               // Timeout requête Formspree (10s)
  const COMMENT_MAX     = 300;                  // Longueur max commentaire
  const NAME_MAX        = 60;                   // Longueur max prénom
  const EMAIL_MAX       = 100;                  // Longueur max email

  // Regex email — vérifie le format, pas l'existence du compte
  const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;

  // Patterns dangereux rejetés dans tous les champs texte libres
  // (balises HTML, protocoles dangereux, event handlers inline)
  ...
  //    Toutes les données sont lues depuis le contexte de la page bloquée.
  // ---------------------------------------------------------------------------
  ...
  /** Extrait le nom et la version du navigateur depuis le user-agent. */
  ...
  // ---------------------------------------------------------------------------
  // 4. STORAGE — Abstraction chrome.storage / localStorage
  // ---------------------------------------------------------------------------

  /** Lit des clés depuis chrome.storage.local (fallback localStorage). */
  ...
  /** Écrit dans chrome.storage.local (fallback localStorage). */
  ...
  // ---------------------------------------------------------------------------
  // 5. MOTIF DE BLOCAGE — Lecture du DOM NextDNS via MutationObserver
  //    NextDNS peuple #lists de façon asynchrone via son propre JS.
  //    On observe les mutations pour capturer la valeur dès qu'elle apparaît.
  // ---------------------------------------------------------------------------
  let blockReason = "";
  ...
  // ---------------------------------------------------------------------------
  // 6. ANTI-DOUBLON — Empêche de re-signaler le même domaine avant 48h
  // ---------------------------------------------------------------------------
  ...
  /** Retourne true si ce domaine a été signalé dans les dernières 48h. */
  ...
  /** Enregistre le signalement et maintient un max de 30 entrées (FIFO). */
  ...
  /** Supprime les caractères pouvant former des injections ou du markup. */
  function sanitize(str) {
    return str.replace(/[<>"'`]/g, "").trim();
  }

  /** Valide le format d'un email (ne vérifie pas son existence). */
  function isValidEmail(v) {
    return v.length > 0 && v.length <= EMAIL_MAX && EMAIL_RE.test(v);
  }

  /** Retourne false si la chaîne contient du code ou des balises dangereuses. */
  function isSafe(v) {
  ...
  style.textContent = `
    ...
    #ndns-fab:disabled { cursor: default; }
    #ndns-fab.ndns-already { background: #92400e; box-shadow: 0 4px 20px rgba(146,64,14,0.4); }
    #ndns-fab.ndns-sent    { background: #0e9f6e; box-shadow: 0 4px 20px rgba(14,159,110,0.4); }
    ...
    .ndns-details-toggle {
      font-size: 11px; color: #4f87e8; cursor: pointer;
    ...
    .ndns-detail-row {
      ...
      margin-bottom: 4px; word-break: break-all; line-height: 1.5;
    }
    .ndns-detail-row b { color: rgba(255,255,255,0.6); font-weight: 500; }

    /* ── Champs ── */
    .ndns-field { margin-bottom: 14px; }
    .ndns-label-row { display: flex; align-items: center; gap: 6px; margin-bottom: 5px; }
    .ndns-label {
      font-size: 12px; font-weight: 600; color: rgba(255,255,255,0.6);
    }
    .ndns-required   { color: #f87171; font-size: 12px; }
    .ndns-opt-tag    { font-size: 11px; color: rgba(255,255,255,0.28); font-style: italic; font-weight: 400; }
    ...
    .ndns-tip-icon {
      ...
      cursor: help; user-select: none; flex: none; font-style: normal; line-height: 1;
    }
    ...
    .ndns-tip-bubble {
      ...
      font-size: 12px; color: rgba(255,255,255,0.82); line-height: 1.5;
      width: 230px; z-index: 10; pointer-events: none;
    ...
    .ndns-input.ndns-err   { border-color: #f87171; }
    .ndns-input.ndns-valid { border-color: #34d399; }
    textarea.ndns-input { resize: none; height: 80px; }

    .ndns-field-err {
      font-size: 11px; color: #f87171; margin-top: 4px;
      min-height: 14px; display: none;
    }
    .ndns-field-err.ndns-show { display: block; }

    .ndns-char-count { font-size: 10px; color: rgba(255,255,255,0.25); text-align: right; margin-top: 3px; }
    ...
    .ndns-mem-label {
      ...
      font-size: 12px; color: rgba(255,255,255,0.55); cursor: pointer; user-select: none;
    }
    .ndns-mem-label input[type=checkbox] {
      appearance: none; width: 16px; height: 16px;
      border: 1px solid rgba(255,255,255,0.3); border-radius: 4px;
      background: transparent; cursor: pointer; position: relative; flex: none;
      transition: background 0.12s, border-color 0.12s;
    }
    .ndns-mem-label input[type=checkbox]:checked {
      background: #1a56db; border-color: #1a56db;
    }
    .ndns-mem-label input[type=checkbox]:checked::after {
    ...
    #ndns-toast {
      font-size: 11px; color: #6ee7b7; margin-top: 4px;
      min-height: 13px; transition: opacity 0.3s;
    }

    /* ── Section chips ── */
    .ndns-section-row {
      display: flex; align-items: center; gap: 6px; margin: 14px 0 7px;
    }
    .ndns-section-label {
      font-size: 12px; font-weight: 600; color: rgba(255,255,255,0.5);
    }
    .ndns-opt-tag-inline { font-size: 11px; color: rgba(255,255,255,0.28); font-style: italic; }
    ...
    #ndns-send-btn.ndns-already-state { background: #92400e; }
    ...
    #ndns-refresh-hint {
      font-size: 11px; color: rgba(255,255,255,0.3); margin-top: 8px; line-height: 1.5;
    }
  `;
  document.head.appendChild(style);

  // ---------------------------------------------------------------------------
  // 9. CONSTRUCTION DU DOM
  //    Les tooltips utilisent du CSS pur (hover/focus) — aucun JS nécessaire.
  //    Les textes des tooltips sont définis ici en JS pour éviter les
  //    variables non résolues dans le HTML statique (bug v1.2).
  // ---------------------------------------------------------------------------

  /** Génère le HTML d'un tooltip accessible (survol + focus clavier). */
  function tip(text) {
    const safe = text.replace(/"/g, "&quot;");
    return `<span class="ndns-tip-wrap">
      <span class="ndns-tip-icon" tabindex="0" role="tooltip" aria-label="${safe}">?</span>
      <span class="ndns-tip-bubble">${text}</span>
    </span>`;
  }

  const commentHint = `Décrivez brièvement pourquoi vous avez besoin d'accéder à ce site (max ${COMMENT_MAX} caractères).`;
  const contextHint = "Indiquer d'où vient le lien aide l'administrateur à prioriser et contextualiser sa décision.";
  const emailHint   = "Votre adresse email permet à l'administrateur de vous répondre et de savoir qui fait la demande.";
  const nameHint    = "Votre prénom aide l'administrateur à personnaliser sa réponse.";
  const memHint     = "Vos coordonnées sont enregistrées uniquement sur cet appareil et pré-remplies automatiquement lors de votre prochain signalement.";

  const detailRows = [
    `<div class="ndns-detail-row"><b>URL :</b> ${pageUrl}</div>`,
    referrer ? `<div class="ndns-detail-row"><b>Source :</b> ${referrer}</div>` : "",
    `<div class="ndns-detail-row" id="ndns-reason-row" style="display:none">
       <b>Motif de blocage :</b> <span id="ndns-reason-val"></span>
     </div>`
  ].join("");
  ...
  <div id="ndns-card">
    ...
    <p class="ndns-subtitle">L'administrateur réseau recevra un email et traitera votre demande.</p>

    <div id="ndns-already-banner"></div>

    <!-- Domaine + détails techniques -->
    ...
    <div class="ndns-field">
      ...
      <div class="ndns-field-err" id="ndns-email-err">
    ...
    <div class="ndns-field">
      <div class="ndns-label-row">
        ...
        <span class="ndns-opt-tag">(facultatif)</span>
        ${tip(nameHint)}
      </div>
      <input class="ndns-input" id="ndns-name" type="text"
             placeholder="Votre prénom"
             maxlength="${NAME_MAX}" autocomplete="given-name"
             aria-describedby="ndns-name-err"/>
      <div class="ndns-field-err" id="ndns-name-err"></div>
    </div>

    <!-- Mémoriser -->
    <div class="ndns-mem-row">
      <label class="ndns-mem-label">
    ...
    <!-- Contexte -->
    <div class="ndns-section-row">
      <span class="ndns-section-label">Contexte</span>
      <span class="ndns-opt-tag-inline">(facultatif)</span>
      ${tip(contextHint)}
    </div>
    <div class="ndns-chips" id="ndns-chips">
      ${CHIPS.map(c =>
        `<button class="ndns-chip" data-id="${c.id}" type="button">${c.label}</button>`
      ).join("")}
    </div>
    ...
    <div class="ndns-field">
      <div class="ndns-label-row">
        ...
        <span class="ndns-opt-tag">(facultatif, max ${COMMENT_MAX} car.)</span>
        ${tip(commentHint)}
      </div>
      <textarea class="ndns-input" id="ndns-comment"
                placeholder="Ex : besoin pour un devis fournisseur, lien dans une newsletter…"
                maxlength="${COMMENT_MAX}" rows="3"
                aria-describedby="ndns-comment-err"></textarea>
      <div class="ndns-char-count" id="ndns-char-count">0 / ${COMMENT_MAX}</div>
      <div class="ndns-field-err" id="ndns-comment-err"></div>
    </div>
    ...
    <div id="ndns-refresh-block">
      <p>Si l'administrateur a déjà traité votre demande, rafraîchissez la page pour vérifier.</p>
      <button id="ndns-refresh-btn" type="button">↺ Rafraîchir la page</button>
      <div id="ndns-refresh-hint">
        Toujours bloqué après rafraîchissement ?<br>
        Appuyez sur <kbd style="background:rgba(255,255,255,0.1);padding:1px 5px;border-radius:3px;font-size:10px">Ctrl+Shift+R</kbd>
  ...
  // ---------------------------------------------------------------------------
  // 11. FOCUS TRAP — Accessibilité clavier dans la modale (requis store Chrome)
  // ---------------------------------------------------------------------------
  ...
  function trapFocus(e) {
    ...
    const els = Array.from(document.getElementById("ndns-card").querySelectorAll(FOCUSABLE));
  ...
  // ---------------------------------------------------------------------------
  // 12. COMPTEUR DE CARACTÈRES
  // ---------------------------------------------------------------------------
  commentArea.addEventListener("input", () => {
    const n = commentArea.value.length;
    charCount.textContent = `${n} / ${COMMENT_MAX}`;
    charCount.className = "ndns-char-count" +
      (n >= COMMENT_MAX ? " ndns-over" : n >= COMMENT_MAX * 0.85 ? " ndns-near" : "");
  });
  ...
  emailInput.addEventListener("input", () => {
    if (emailInput.classList.contains("ndns-err")) validateEmail(false);
  });

  function validateEmail(showErr) {
    const v = emailInput.value.trim();
    const ok = isValidEmail(v) && isSafe(v);
    emailInput.classList.toggle("ndns-err",   !ok && v !== "");
    emailInput.classList.toggle("ndns-valid",  ok);
    if (showErr && !ok && v !== "") emailErr.classList.add("ndns-show");
    else if (ok)                    emailErr.classList.remove("ndns-show");
    return ok;
  }
  ...
  // ---------------------------------------------------------------------------
  // 14. TOAST DE CONFIRMATION (mémorisation)
  // ---------------------------------------------------------------------------
  let toastTimer = null;
  ...
  // ---------------------------------------------------------------------------
  // 15. DÉTAILS TECHNIQUES — Toggle + affichage du motif de blocage
  // ---------------------------------------------------------------------------
  ...
  function updateReason() {
    if (blockReason) {
      reasonVal.textContent = blockReason;
      reasonRow.style.display = "block";
    }
  }
  // Appel différé — NextDNS peuple #lists de façon asynchrone
  setTimeout(updateReason, 1500);

  // ---------------------------------------------------------------------------
  // 16. CHIPS DE CONTEXTE
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // 17. OUVRIR / FERMER LA MODALE
  // ---------------------------------------------------------------------------
  async function openModal() {
    const already = await wasRecentlyReported(domain);
    if (already) {
      alreadyBanner.innerHTML = `⚠ <strong>Déjà signalé.</strong>
        Votre demande est en cours de traitement. Merci de patienter ou de
        contacter directement l'administrateur.`;
      alreadyBanner.classList.add("ndns-open");
      sendBtn.disabled = true;
      sendBtn.classList.add("ndns-already-state");
      sendBtn.innerHTML = "⏳ Demande déjà envoyée";
    } else {
    ...
    setTimeout(() => {
      const f = document.getElementById("ndns-card").querySelector(FOCUSABLE);
      if (f) f.focus();
    }, 50);
  }
  ...
  fab.addEventListener("click", async () => {
    ...
    // Pré-remplissage depuis le storage
    if (data.ndns_email)    { emailInput.value = data.ndns_email;    validateEmail(false); }
    if (data.ndns_username) nameInput.value  = data.ndns_username;
    if (data.ndns_email || data.ndns_username) rememberChk.checked = true;

    // Mode one-click : envoi immédiat si email mémorisé valide
    if (data.ndns_oneclic) {
      if (!data.ndns_email || !isValidEmail(data.ndns_email)) {
        await openModal(); // Pas d'email → ouvrir quand même
        return;
      }
      const already = await wasRecentlyReported(domain);
      if (already) {
        fab.innerHTML = "⏳ Déjà signalé";
        fab.classList.add("ndns-already");
        fab.disabled = true;
        setTimeout(() => {
          fab.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 2L11 13"/><path d="M22 2L15 22l-4-9-9-4 20-7z"/></svg> Demander le déblocage`;
  ...
  refreshBtn.addEventListener("click", () => {
    // Vide le cache navigateur et recharge — équivalent Ctrl+Shift+R
    location.reload(true);
  });

  sendBtn.addEventListener("click", async () => {
    if (!validateEmail(true) || !validateName() || !validateComment()) return;
  ...
  // 19. ENVOI
  // ---------------------------------------------------------------------------
  let sending = false; // Verrou anti-double-clic

  async function sendReport({ email, name, context, comment, endpoint }) {
    if (sending) return;
    sending = true;

    // Résolution de l'endpoint (ID seul ou URL complète)
    const data = await storageGet(["ndns_endpoint"]);
    ...
    // ── Mise à jour UI ──
    const fabOrigHTML = fab.innerHTML;
    fab.innerHTML = "<span>Envoi…</span>";
    fab.disabled  = true;
    sendBtn.disabled = true;
    ...
    const payload = {
      ...
      "Source / referrer": referrer || "Accès direct",
    ...
    try {
      ...
      // Mémorisation si cochée
      if (rememberChk.checked) {
        storageSet({ ndns_email: email, ndns_username: name });
        if (email || name) showToast("✓ Coordonnées mémorisées sur cet appareil");
      }

      fab.innerHTML = "✓ Demande envoyée";
      fab.classList.add("ndns-sent");
      fab.disabled = true;

      sendBtn.innerHTML = "✓ Demande envoyée !";
      ...
      // ── Échec — restaurer l'UI ──
      fab.innerHTML = fabOrigHTML;
      fab.disabled  = false;
      sendBtn.innerHTML = "⚠ Erreur — réessayer";
      sendBtn.classList.add("ndns-error");
      sendBtn.disabled  = false;
      statusEl.textContent = isTimeout
        ? "Le serveur ne répond pas (timeout 10s). Vérifiez votre connexion."
  ...
  // ---------------------------------------------------------------------------
  (async () => {
    // Vérification silencieuse si le domaine a déjà été signalé
    // (pour griser le FAB dès le chargement si nécessaire)
    const already = await wasRecentlyReported(domain);
    if (already) {
      fab.innerHTML = "⏳ Déjà signalé";
      fab.classList.add("ndns-already");
      // On laisse quand même le bouton cliquable pour voir la bannière
    }
  })();
